...
///
/// // 转移所有权
/// let process = Process::new(pid_handle);
///
/// // 当 process 被销毁时，PID 会自动回收
/// ```
...
/// 和进程切换时的临时数据。每个内核栈在虚拟地址空间中占用固定的位置。
///
/// ## 设计特性
///
/// ### 栈空间隔离
...
impl KernelStack {
    ...
    /// 可以压入任何实现了 `Sized` trait 的类型。主要用于在进程切换时保存上下文信息。
    ///
    /// ## Type Parameters
    ///
