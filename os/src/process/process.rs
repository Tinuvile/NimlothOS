//! # 进程控制块模块
//!
//! 提供进程控制块 (Process Control Block, PCB) 的实现，管理进程的完整生命周期。
//! 支持进程创建、fork、exec、状态管理等核心操作，是操作系统进程管理的核心数据结构。
//!
//! ## 核心组件
//!
//! - [`ProcessControlBlock`] - 进程控制块，包含进程的所有信息
//! - [`ProcessControlBlockInner`] - PCB 内部可变部分，受互斥锁保护
...
/// PCB 采用内外分离设计，将不变和可变部分分开管理，提高并发访问效率。
///
/// ## 结构组成
///
/// ### 不变部分（直接字段）
...
impl ProcessControlBlockInner {
    ...
    /// 僵尸进程不会被调度执行，但保留 PCB 以供父进程获取退出信息。
    ///
    /// ## Returns
    ///
