//! # 任务管理器模块
//!
//! 提供任务调度和管理的核心功能，实现就绪队列的维护和任务分发。
//! 采用 FIFO (First In, First Out) 调度策略，确保任务调度的公平性和可预测性。
//!
//! ## 核心组件
//!
//! - [`TaskManager`] - 任务管理器，维护就绪任务队列
//! - [`TASK_MANAGER`] - 全局任务管理器实例
//! - [`add_task`] - 向就绪队列添加任务的全局接口
//! - [`fetch_task`] - 从就绪队列获取任务的全局接口
//!
//! ## 设计原理
//!
//! ### 调度策略
//!
//! 采用简单而高效的 **先进先出 (FIFO)** 调度算法：
//! - **公平性**：所有任务按照进入就绪队列的顺序被调度
//! - **简洁性**：避免复杂的优先级计算和时间片管理
//! - **可预测性**：调度行为完全可预测，便于调试和测试
//!
//! ### 数据结构选择
//!
//! 使用 `VecDeque<Arc<TaskControlBlock>>` 作为就绪队列：
//! - **双端队列**：支持高效的队首删除和队尾插入操作
//! - **引用计数**：`Arc` 允许多个调度器组件共享任务控制块
//! - **时间复杂度**：入队和出队操作均为 O(1)
//!
//! ### 并发安全
//!
//! 通过 `UPSafeCell` 实现单处理器环境下的线程安全：
//! - **互斥访问**：确保同一时间只有一个线程可以修改就绪队列
//! - **内部可变性**：允许在不可变引用下修改内部数据
//! - **无锁设计**：在单核环境下避免昂贵的锁操作
//!
//! ## 调度流程
//!
//! ```text
//! Task Creation/Wake-up          Task Scheduling
//!         │                           │
//!         ▼                           ▼
//! ┌──────────────┐           ┌──────────────┐
//! │   add_task   │           │ fetch_task   │
//! │  (Enqueue)   │           │  (Dequeue)   │
//! └──────┬───────┘           └──────┬───────┘
//!        │                          │
//!        ▼                          ▼
//! ┌───────────────────────────────────────-----──-─┐
//! │            Ready Queue                         │
//! │  ┌─────┐  ┌─────┐  ┌─────┐  ┌─────┐            │
//! │  │Task │◄─│Task │◄─│Task │◄─│Task │            │
//! │  │  A  │  │  B  │  │  C  │  │  D  │            │
//! │  └─────┘  └─────┘  └─────┘  └─────┘            │
//! │    ▲                           │               │
//! │    │ Add New Task              │ Get Next Task │
//! │    └────────────────────────-──┘               │
//! └─────────────────────────────────────────------─┘
//! ```
//!
//! ## 性能特征
//!
//! - **入队延迟**: O(1) - 常数时间复杂度
//! - **出队延迟**: O(1) - 常数时间复杂度  
//! - **内存开销**: 每个任务一个 `Arc` 指针 (8 bytes)
//! - **缓存友好性**: 连续内存布局提供良好的缓存局部性
//!
//! ## 使用示例
//!
//! ```rust
//! use crate::task::manager::{add_task, fetch_task};
//! use crate::task::task::TaskControlBlock;
//! use alloc::sync::Arc;
//!
//! // 添加新任务到就绪队列
//! let new_task = Arc::new(TaskControlBlock::new(app_data));
//! add_task(new_task);
//!
//! // 调度器获取下一个任务
//! if let Some(task) = fetch_task() {
//!     println!("调度任务 PID: {}", task.getpid());
//!     // 执行任务...
//! } else {
//!     println!("没有就绪任务，进入空闲状态");
//! }
//! ```

use crate::sync::UPSafeCell;
use crate::task::task::TaskControlBlock;
use alloc::{
    collections::{BTreeMap, vec_deque::VecDeque},
    sync::Arc,
};
use lazy_static::lazy_static;

/// 任务管理器
///
/// 负责维护系统中所有就绪态任务的队列，提供高效的任务调度服务。
/// 采用 FIFO 调度策略，确保任务调度的公平性和可预测性。
///
/// ## 设计特点
///
/// ### 数据结构
/// - **就绪队列**: 使用 `VecDeque` 实现双端队列，支持 O(1) 的队首删除和队尾插入
/// - **任务引用**: 存储 `Arc<TaskControlBlock>` 而非任务本身，支持共享所有权
/// - **内存布局**: 连续内存分配提供良好的缓存局部性
///
/// ### 调度算法
/// ```text
/// Schedule Order: Task1 → Task2 → Task3 → Task4
///          ┌─────┐  ┌─────┐  ┌─────┐  ┌─────┐
/// Enqueue ►│Task4│◄─│Task3│◄─│Task2│◄─│Task1│◄─── Dequeue
///          └─────┘  └─────┘  └─────┘  └─────┘
///         (最新)                      (最老)
/// ```
///
/// ### 线程安全
/// 虽然 `TaskManager` 本身不提供线程安全保证，但通过全局的 `UPSafeCell`
/// 包装器确保在单处理器环境下的并发安全访问。
///
/// ## 使用模式
///
/// 通常不直接实例化 `TaskManager`，而是通过全局函数接口使用：
/// ```rust
/// // 推荐方式：通过全局接口使用
/// add_task(new_task);
/// let next_task = fetch_task();
///
/// // 不推荐：直接操作实例
/// let mut manager = TaskManager::new();
/// manager.add(task);
/// ```
///
/// ## 性能保证
///
/// - **入队延迟**: O(1) 常数时间
/// - **出队延迟**: O(1) 常数时间
/// - **内存开销**: 8 bytes per task (仅存储 Arc 指针)
/// - **扩容策略**: 按需自动扩容，避免频繁内存分配
pub struct TaskManager {
    /// 就绪任务队列
    ///
    /// 使用双端队列存储所有处于就绪状态的任务控制块。
    /// 队列头部是最早加入的任务（下次将被调度），
    /// 队列尾部是最新加入的任务。
    ///
    /// ## 队列特性
    /// - **FIFO 顺序**: 先进先出，保证调度公平性
    /// - **动态大小**: 根据系统负载自动调整容量
    /// - **高效操作**: 队首删除和队尾插入均为 O(1)
    ready_queue: VecDeque<Arc<TaskControlBlock>>,
}

impl TaskManager {
    /// 创建新的任务管理器实例
    ///
    /// 初始化一个空的就绪队列，准备接收和调度任务。
    /// 新创建的管理器不包含任何任务，队列容量为 0。
    ///
    /// ## 返回值
    ///
    /// 返回一个新的 `TaskManager` 实例，包含：
    /// - 空的就绪队列（容量为 0）
    /// - 所有计数器重置为 0
    ///
    /// ## 性能特征
    ///
    /// - **时间复杂度**: O(1) - 常数时间初始化
    /// - **内存分配**: 不进行堆内存分配
    /// - **初始容量**: 0，按需扩容以优化内存使用
    ///
    /// ## 使用说明
    ///
    /// 通常不需要直接调用此函数，而是通过全局 `TASK_MANAGER` 实例使用：
    ///
    /// ```rust
    /// // 推荐：使用全局实例
    /// use crate::task::manager::{add_task, fetch_task};
    ///
    /// // 不推荐：直接创建实例
    /// let manager = TaskManager::new();
    /// ```
    ///
    /// ## 线程安全
    ///
    /// `new()` 函数本身是线程安全的，但返回的实例需要通过 `UPSafeCell`
    /// 等同步原语保护才能在多线程环境中安全使用。
    pub fn new() -> Self {
        Self {
            ready_queue: VecDeque::new(),
        }
    }

    /// 向就绪队列添加任务
    ///
    /// 将指定的任务控制块添加到就绪队列的尾部，使其可以被调度器选中执行。
    /// 新添加的任务将按照 FIFO 顺序等待调度。
    ///
    /// ## 参数
    ///
    /// * `task` - 要添加到队列的任务控制块，包装在 `Arc` 中支持共享所有权
    ///
    /// ## 调度策略
    ///
    /// 任务按照加入队列的顺序被调度：
    ///
    /// ```text
    /// 调度顺序 (先进先出):
    ///
    /// add(Task1) ──► add(Task2) ──► add(Task3)
    ///      │             │             │
    ///      ▼             ▼             ▼
    /// ┌─────────┐   ┌─────────┐   ┌─────────┐
    /// │ Task1   │◄──│ Task2   │◄──│ Task3   │
    /// │ (First) │   │(Second) │   │ (Last)  │
    /// └─────────┘   └─────────┘   └─────────┘
    ///      ▲                           ▲
    ///      │                           │
    ///   fetch()                     最新添加
    /// ```
    ///
    /// ## 性能特征
    ///
    /// - **时间复杂度**: O(1) 摊还常数时间
    /// - **内存分配**: 队列满时可能触发 O(n) 的扩容操作
    /// - **扩容策略**: 通常按 2 倍容量扩展
    ///
    /// ## 任务状态要求
    ///
    /// 添加到就绪队列的任务通常应该满足：
    /// - 任务状态为 `TaskStatus::Ready`
    /// - 拥有有效的内存地址空间
    /// - 陷阱上下文已正确初始化
    ///
    /// ## 使用示例
    ///
    /// ```rust
    /// use alloc::sync::Arc;
    /// use crate::task::task::TaskControlBlock;
    ///
    /// let task = Arc::new(TaskControlBlock::new(app_data));
    /// manager.add(task);
    ///
    /// // 任务现在在就绪队列中等待调度
    /// println!("任务已添加到就绪队列");
    /// ```
    ///
    /// ## 并发考虑
    ///
    /// 在并发环境中，应通过适当的同步机制保护此操作：
    ///
    /// ```rust
    /// // 全局接口已提供同步保护
    /// add_task(task); // 推荐方式
    ///
    /// // 直接使用需要手动同步
    /// TASK_MANAGER.exclusive_access().add(task);
    /// ```
    pub fn add(&mut self, task: Arc<TaskControlBlock>) {
        self.ready_queue.push_back(task);
    }

    /// 从就绪队列获取下一个待调度任务
    ///
    /// 从队列头部移除并返回最早加入的任务，实现 FIFO 调度策略。
    /// 如果队列为空，返回 `None`。
    ///
    /// ## 返回值
    ///
    /// * `Some(Arc<TaskControlBlock>)` - 成功获取到任务控制块
    /// * `None` - 就绪队列为空，没有可调度的任务
    ///
    /// ## 调度行为
    ///
    /// 按照任务加入队列的顺序进行调度：
    ///
    /// ```text
    /// 队列状态变化:
    ///
    /// 调用前:
    /// ┌─────────┐   ┌─────────┐   ┌─────────┐
    /// │ Task1   │◄──│ Task2   │◄──│ Task3   │
    /// │(Oldest) │   │         │   │(Latest) │
    /// └─────────┘   └─────────┘   └─────────┘
    ///      ▲
    ///   fetch() 返回 Task1
    ///
    /// 调用后:
    /// ┌────---─────┐   ┌─────────┐
    /// │   Task2    │◄──│ Task3   │
    /// │(Now Oldest)│   │(Latest) │
    /// └───────---──┘   └─────────┘
    /// ```
    ///
    /// ## 性能特征
    ///
    /// - **时间复杂度**: O(1) 常数时间
    /// - **内存操作**: 仅移动指针，不涉及数据拷贝
    /// - **缓存效率**: 访问队列头部具有良好的缓存局部性
    ///
    /// ## 空队列处理
    ///
    /// 当队列为空时安全返回 `None`，调用者应适当处理：
    ///
    /// ```rust
    /// match manager.fetch() {
    ///     Some(task) => {
    ///         println!("获取到任务 PID: {}", task.getpid());
    ///         // 执行任务调度...
    ///     }
    ///     None => {
    ///         println!("没有就绪任务，CPU 进入空闲状态");
    ///         // 可能触发节能模式或等待中断
    ///     }
    /// }
    /// ```
    ///
    /// ## 任务生命周期
    ///
    /// 从队列中取出的任务通常会：
    /// 1. 状态从 `Ready` 转变为 `Running`
    /// 2. 被加载到 CPU 执行
    /// 3. 根据执行结果重新加入队列或结束
    ///
    /// ## 调度公平性
    ///
    /// FIFO 策略保证了调度的公平性：
    /// - 所有任务都有相同的调度机会
    /// - 不会出现任务饥饿问题
    /// - 调度行为完全可预测
    ///
    /// ## 使用示例
    ///
    /// ```rust
    /// // 调度器主循环
    /// loop {
    ///     if let Some(task) = manager.fetch() {
    ///         // 切换到任务执行
    ///         run_task(task);
    ///     } else {
    ///         // 没有任务，等待事件
    ///         wait_for_interrupt();
    ///     }
    /// }
    /// ```
    pub fn fetch(&mut self) -> Option<Arc<TaskControlBlock>> {
        self.ready_queue.pop_front()
    }
}

lazy_static! {
    /// 全局任务管理器实例
    ///
    /// 系统唯一的任务管理器实例，负责维护所有就绪任务的全局状态。
    /// 使用 `UPSafeCell` 包装以提供在单处理器环境下的线程安全访问。
    ///
    /// ## 设计特点
    ///
    /// ### 单例模式
    /// - **全局唯一**: 整个系统只有一个任务管理器实例
    /// - **集中管理**: 所有任务调度操作都通过此实例进行
    /// - **状态一致**: 保证系统任务调度状态的一致性
    ///
    /// ### 并发安全
    /// - **UPSafeCell**: 单处理器环境下的内部可变性
    /// - **互斥访问**: 通过 `exclusive_access()` 获得互斥访问权
    /// - **无竞争**: 在单核系统中避免复杂的锁机制
    ///
    /// ### 延迟初始化
    /// - **lazy_static**: 在首次访问时才执行初始化
    /// - **零成本**: 不会在启动时增加领外开销
    /// - **线程安全**: 初始化过程本身由 lazy_static 保证安全
    ///
    /// ## 内存布局
    ///
    /// ```text
    /// 全局内存区域:
    /// ┌────────────────────────────────────────┐
    /// │              TASK_MANAGER              │
    /// ├────────────────────────────────────────┤
    /// │          UPSafeCell<TaskManager>       │
    /// │  ┌───────────────────────────-─────-┐  │
    /// │  │          TaskManager             │  │
    /// │  │ ┌──────────────────────────────┐ │  │
    /// │  │ │      VecDeque<Arc<TCB>>      │ │  │
    /// │  │ │    [Task1][Task2][Task3]...  │ │  │
    /// │  │ └──────────────────────────────┘ │  │
    /// │  └─────────────────────────────-───-┘  │
    /// └────────────────────────────────────────┘
    /// ```
    ///
    /// ## 访问模式
    ///
    /// **直接访问**（不推荐）：
    /// ```rust
    /// // 危险！需要手动管理锁
    /// let mut manager = TASK_MANAGER.exclusive_access();
    /// manager.add(task);
    /// drop(manager); // 必须显式释放
    /// ```
    ///
    /// **推荐方式**（通过全局函数）：
    /// ```rust
    /// // 安全！自动管理锁生命周期
    /// add_task(task);
    /// let next_task = fetch_task();
    /// ```
    ///
    /// ## 初始化时机
    ///
    /// - **首次访问**: 在第一次调用 `add_task` 或 `fetch_task` 时初始化
    /// - **延迟加载**: 不会影响系统启动速度
    /// - **一次性**: 初始化完成后不会重复执行
    ///
    /// ## 性能影响
    ///
    /// - **全局访问**: 不涉及页表切换，访问速度很快
    /// - **单点争用**: 所有调度操作都会经过此实例
    /// - **缓存局部性**: 全局变量在内存中位置相对固定
    ///
    /// ## 线程安全保证
    ///
    /// - **互斥访问**: `exclusive_access()` 保证同一时间只有一个线程可以修改
    /// - **内存顺序**: 在单核系统中保证操作的原子性
    /// - **无死锁**: 简单的所有权模型避免了复杂的锁依赖
    pub static ref TASK_MANAGER: UPSafeCell<TaskManager> =
        unsafe { UPSafeCell::new(TaskManager::new()) };

    pub static ref PID2TCB: UPSafeCell<BTreeMap<usize, Arc<TaskControlBlock>>> =
        unsafe { UPSafeCell::new(BTreeMap::new()) };
}

/// 向全局就绪队列添加任务
///
/// - 将 `task` 写入 `PID2TCB` 映射，便于通过 PID 查找 TCB
/// - 将任务入队到全局 `TASK_MANAGER` 的就绪队列，等待调度
///
/// ## 参数
/// * `task` - 待加入调度的任务控制块
///
/// ## 复杂度
/// - O(1) 摊还时间；队列扩容时可能触发 O(n) 拷贝
pub fn add_task(task: Arc<TaskControlBlock>) {
    PID2TCB
        .exclusive_access()
        .insert(task.getpid(), Arc::clone(&task));
    TASK_MANAGER.exclusive_access().add(task);
}

/// 通过 PID 查询任务控制块
///
/// ## 参数
/// * `pid` - 目标进程标识符
///
/// ## 返回
/// - `Some(Arc<TaskControlBlock>)`：找到对应任务
/// - `None`：不存在该 PID 的任务（可能已退出并被回收）
pub fn pid2task(pid: usize) -> Option<Arc<TaskControlBlock>> {
    let map = PID2TCB.exclusive_access();
    map.get(&pid).map(Arc::clone)
}

/// 从全局 PID → TCB 映射中移除任务
///
/// 典型调用点：
/// - 进程退出路径（如 `exit_current_and_run_next` / `waitpid` 回收）
///
/// ## 参数
/// * `pid` - 将要移除的进程标识符
///
/// ## 行为
/// - 若不存在该 PID，触发 panic，用于暴露流程一致性问题
pub fn remove_from_pid2task(pid: usize) {
    let mut map = PID2TCB.exclusive_access();
    if map.remove(&pid).is_none() {
        panic!("cannot find pid {} in pid2task!", pid);
    }
}

/// 从全局任务管理器获取下一个待调度任务
///
/// 调度器核心函数，从就绪队列中获取最早加入的任务进行执行。
/// 实现 FIFO 调度策略，确保任务调度的公平性。
///
/// ## 返回值
///
/// * `Some(Arc<TaskControlBlock>)` - 成功获取到一个就绪任务
/// * `None` - 就绪队列为空，没有可调度的任务
///
/// ## 调度策略
///
/// 采用先进先出 (FIFO) 策略：
///
/// ```text
/// 调度顺序示意图:
///
/// 时间轴: T1    T2    T3    T4
///         │     │     │     │
///         ▼     ▼     ▼     ▼
///      add(A) add(B) add(C) add(D)
///         │     │     │     │
///         ▼     ▼     ▼     ▼
/// 队列: [A] ─► [B|A] [C|B|A] [D|C|B|A]
///         │           │         │
///         ▼           ▼         ▼
///     fetch()    fetch()   fetch()
///      返回A       返回B     返回C
/// ```
///
/// ## 使用场景
///
/// ### 主调度循环
/// ```rust
/// use crate::task::manager::fetch_task;
///
/// loop {
///     match fetch_task() {
///         Some(task) => {
///             println!("调度任务 PID: {}", task.getpid());
///             run_task(task);
///         }
///         None => {
///             println!("CPU 进入空闲状态");
///             wait_for_interrupt();
///         }
///     }
/// }
/// ```
///
/// ### 批量处理
/// ```rust
/// // 一次取出多个任务进行批量处理
/// let mut tasks = Vec::new();
/// while let Some(task) = fetch_task() {
///     tasks.push(task);
///     if tasks.len() >= BATCH_SIZE {
///         break;
///     }
/// }
/// process_tasks_batch(tasks);
/// ```
///
/// ### 空闲检测
/// ```rust
/// if fetch_task().is_none() {
///     println!("系统无任务，可以进入节能模式");
///     enter_power_save_mode();
/// }
/// ```
///
/// ## 执行流程
///
/// ```text
/// fetch_task 调用流程:
///
/// Scheduler Code
///      │
///      ▼
/// ┌─────────────────┐
/// │  fetch_task()   │ ─── 全局函数接口
/// └────────┬────────┘
///          │
///          ▼
/// ┌─────────────────-┐
/// │exclusive_access()│ ─── 获取互斥访问权
/// └────────┬────────-┘
///          │
///          ▼
/// ┌─────────────────┐
/// │ manager.fetch() │ ─── 从队列头部取出
/// └────────┬────────┘
///          │
///          ▼
/// ┌─────────────────┐
/// │ automatic unlock│ ─── 返回后自动释放
/// └─────────────────┘
///          │
///          ▼
///   Option<Arc<TCB>>
/// ```
///
/// ## 性能特征
///
/// - **时间复杂度**: O(1) 常数时间操作
/// - **内存开销**: 仅转移 Arc 所有权，不复制数据
/// - **锁竞争**: 在单核系统中几乎无开销
/// - **缓存友好**: 访问顺序性数据结构
///
/// ## 空队列处理策略
///
/// 当没有可调度任务时的常见处理方式：
///
/// ```rust
/// match fetch_task() {
///     Some(task) => {
///         // 正常调度流程
///         switch_to_task(task);
///     }
///     None => {
///         // 选择合适的空闲处理策略
///         idle_task();           // 执行空闲任务
///         // halt_cpu();         // 挂起 CPU 等待中断
///         // power_management();  // 进入节能模式
///     }
/// }
/// ```
///
/// ## 调度公平性
///
/// FIFO 策略保证了：
/// - **无饥饿**: 没有任务会被无限期推迟
/// - **公平性**: 所有任务都有相等的被调度机会
/// - **可预测**: 调度顺序完全可预测
///
/// ## 错误处理
///
/// 此函数不会返回错误，但可能在以下情况下 panic：
/// - 全局管理器未正确初始化
/// - 队列数据结构损坏（极罕见）
///
/// ## 线程安全
///
/// - **自动锁管理**: 函数进入时自动加锁，返回时自动解锁
/// - **互斥性**: 与 `add_task` 互斥，不会产生竞争条件
/// - **异常安全**: 在 panic 发生时也能正确释放锁
///
/// ## 性能优化建议
///
/// 对于高频调用场景，考虑批量获取优化：
///
/// ```rust
/// // 简单但低效的方式
/// for _ in 0..100 {
///     if let Some(task) = fetch_task() {
///         process_task(task);
///     }
/// }
///
/// // 更高效的批量处理
/// let tasks = {
///     let mut manager = TASK_MANAGER.exclusive_access();
///     let mut batch = Vec::new();
///     while let Some(task) = manager.fetch() {
///         batch.push(task);
///         if batch.len() >= 100 { break; }
///     }
///     batch
/// };
/// for task in tasks {
///     process_task(task);
/// }
/// ```
pub fn fetch_task() -> Option<Arc<TaskControlBlock>> {
    TASK_MANAGER.exclusive_access().fetch()
}
